...
grafico1<-ggplot(data=ds,mapping=aes(x=ds$MinTemp,y=ds$MaxTemp))
grafico1<-grafico1+geom_point()
print(grafico1)
grafico2<-ggplot(data=ds,mapping=aes(x=ds$MinTemp,y=ds$MaxTemp,colour=ds$RainTomorrow))
grafico2<-grafico2+geom_point()
print(grafico2)
grafico3<-grafico2+ggtitle("Temperatura Mínima vs. Máxima, coloreado por Lluvia Mañana")
print(grafico3)
...
rm(list=ls())
load("data/weatherAUS.RData")
grafico4<-ggplot(data=ds,mapping=aes(x=ds$Temp9am,y=ds$Pressure9am))
grafico4<-grafico4+geom_point()
print(grafico4)
grafico_shape<-ggplot(data=ds,mapping=aes(x=ds$Temp9am,y=ds$Pressure9am,shape=ds$RainToday))
grafico_shape<-grafico_shape+geom_point()
print(grafico_shape)
grafico_colour<-ggplot(data=ds,mapping=aes(x=ds$Temp9am,y=ds$Pressure9am,colour=ds$RainToday))
grafico_colour<-grafico_colour+geom_point()
print(grafico_colour)
print("La representación con el color permite distinguir mejor entre las categorías de 'RainToday', sobre todo en gráficos como este donde hay muchos puntos e incluso superpuestos.")
grafico_colour<-grafico_colour+labs(title = "Temperatura y Presión Atmosférica a las 9 am",subtitle = "Análisis del impacto de la lluvia hoy",
...
str(iris)
iris.grafico1<-ggplot(data=iris.tidy,mapping=aes(x=iris.tidy$Species,y=iris.tidy$Value))
iris.grafico1<-iris.grafico1+geom_point() 
print(iris.grafico1)
iris.grafico2<-ggplot(data=iris.tidy,mapping=aes(x=iris.tidy$Species,y=iris.tidy$Value,colour=iris.tidy$Part))
iris.grafico2<-iris.grafico2+geom_point() 
print(iris.grafico2)
iris.grafico_filas <- iris.grafico2 +facet_grid(iris.tidy$Measure~.) + ggtitle("Comparación de Medidas (Facetas en Filas)")
print(iris.grafico_filas)
iris.grafico_columnas <- iris.grafico2 +facet_grid(.~iris.tidy$Measure) + ggtitle("Comparación de Medidas (Facetas en Columnas)")
print(iris.grafico_columnas)
print("Para comparar los valores de las dos variables ('Length' y 'Width') de 'Measure', la disposición en columnas es mejor ya que contamos con una unica escala de eje Y (Value), permitiendo una comparación directa de cada variable para cada especie.")
iris.grafico2<-ggplot(data=iris.tidy,mapping=aes(x=iris.tidy$Species,y=iris.tidy$Value,colour=iris.tidy$Part))
iris.grafico_columnas <- iris.grafico2 +facet_grid(.~iris.tidy$Measure) + ggtitle("Comparación de Medidas (Facetas en Columnas)")
iris.grafico_columnas<-iris.grafico_columnas+geom_jitter()
print(iris.grafico_columnas)
print("Con geom_point(), al haber muchos puntos en las variables categóricas ('Species'), los puntos se superponen completamente, formando líneas sólidas o bloques ocultando la verdadera distribución y cantidad de observaciones.")
print("Con geom_jitter(), se añade un pequeño valor aleatorio (ruido) a la posición de los puntos (especialmente en el eje X, que es categórico aquí). Esto dispersa ligeramente los puntos superpuestos, revelando la densidad y la distribución real de las observaciones en la variable continua ('Value') para cada categoría.")
...
str(DatosEjer4)
ggej4<-ggplot(data=DatosEjer4,mapping=aes(x=DatosEjer4$Fecha,y=DatosEjer4$Ventas,colour=DatosEjer4$Fabricante))+geom_point()
print(ggej4)
print("Este gráfico no sirve para evaluar la evolucion de las ventas segun el fabricante porque solo son puntos dispersos, deberiamos hacer un grafico de dispersion con union de los puntos de cada tipo")
ggej4_lineas<-ggplot(data=DatosEjer4,mapping=aes(x=DatosEjer4$Fecha,y=DatosEjer4$Ventas,colour=DatosEjer4$Fabricante))+geom_point()+geom_line()
print(ggej4_lineas)
ggej4_lineas_dif<-ggplot(data=DatosEjer4,mapping=aes(x=DatosEjer4$Fecha,y=DatosEjer4$Ventas,colour=DatosEjer4$Fabricante,linetype=DatosEjer4$Fabricante))+geom_point()+geom_line()
print(ggej4_lineas_dif)
ggej4_lineas_dif<-ggej4_lineas_dif+ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
print(ggej4_lineas_dif)
...
str(Experimento)
ggej5<-ggplot(data=Experimento,mapping=aes(x=Experimento$Dosis,y=Experimento$Altura,linetype=Experimento$Complemento,shape=Experimento$Complemento))+geom_point()+geom_line()
print(ggej5)
ggej5_error<-ggplot(data=Experimento,mapping=aes(x=Experimento$Dosis,y=Experimento$Altura,linetype=Experimento$Complemento,shape=Experimento$Complemento))+geom_point()+geom_line()+geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))
print(ggej5_error)
ggej5_red<-ggplot(data=Experimento,mapping=aes(x=Experimento$Dosis,y=Experimento$Altura,colour=Complemento))+geom_point(aes(linetype=Experimento$Complemento,shape=Experimento$Complemento))+geom_line(aes(linetype=Experimento$Complemento,shape=Experimento$Complemento))+geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))+scale_color_brewer(palette = "Reds")
print(ggej5_red)
...
str(ice.river.df)
library(latex2exp)
ggej6<-ggplot(data=ice.river.df,mapping=aes(x=time))+
# Capa para flow.vat (Rojo)
geom_line(aes(y = flow.vat), colour = "red") +
# Capa para flow.jok (Azul)
geom_line(aes(y = flow.jok), colour = "blue") +
#labs para enriquecer el gráfico
labs(title = "Caudal Diario de los Ríos Skafta y Jokulsa",
subtitle = paste0("Serie temporal: ", min(ice.river.df$time), " - ", max(ice.river.df$time)),
x = "Tiempo (Años)",
y = TeX('$Caudal (m^3/s)$'))
print(ggej6)
...
str(ds)
boxplot1<-ggplot(data=ds,mapping=aes(x=Year,y=MinTemp))+geom_boxplot()+labs(title = "Distribución de Temperaturas Mínimas por Año",x = "Año",y = "Temperatura Mínima (ºC)") 
print(boxplot1)
violin1<-ggplot(data=ds,mapping=aes(x=Year,y=MaxTemp,fill=Year))+geom_violin()+labs(title="Distribución de temperaturas máximas por año",x="Año",y="Temperatura Máxima (ºC)")
print(violin1)
violin_final <- violin1 +guides(fill = "none") +
# Inclinar las etiquetas del eje X 45 grados para evitar superposición.
# Esto se hace dentro de la capa theme().
theme(axis.text.x = element_text(angle = 45, hjust = 1))
# angle: ángulo de inclinación (45 grados)
# hjust: justificación horizontal (1 para alinear el final del texto con la marca del tick)
print(violin_final)     
...
#borrar todas las variables
rm(list=ls())
url_data <- "http://assets.datacamp.com/blog_assets/chol.txt"
file_name <- "chol.txt"
download.file(url_data, file_name)
df <- read.table(file_name, header = TRUE)
str(df)
histog_base <- ggplot(data = df, aes(x = HEIGHT)) +geom_histogram()
print(histog_base)
histog_base <- histog_base+labs(title = "Histograma de la variable Altura",x = "Altura (cm)",y = "Frecuencia") 
print(histog_base)
# Creamos una variable para los 'breaks'
# Rango [155, 195], 20 bins -> ancho de bin = 2
bin_breaks <- seq(155, 195, by = 2)
histog_base <- ggplot(data = df, aes(x = HEIGHT))+labs(title = "Histograma de la variable Altura",x = "Altura (cm)",y = "Frecuencia") +geom_histogram(breaks = bin_breaks)
print(histog_base)
histog_base <- ggplot(data = df, aes(x = HEIGHT))+labs(title = "Histograma de la variable Altura",x = "Altura (cm)",y = "Frecuencia") +geom_histogram(breaks = bin_breaks,col = "red",fill = "blue", alpha = 0.5 )
print(histog_base)
# Es fundamental usar aes(y = after_stat(density)) en ambas capas para normalizar
grafico_con_densidad <- ggplot(data = df, aes(x = HEIGHT)) +
# Histograma (usando density en Y para que coincida con la curva)
#La densidad es la frecuencia normalizada de tal forma que el área total bajo la curva suma 1. El resultado es un gráfico donde la curva de densidad se superpone perfectamente sobre el histograma, ya que ambos están en la misma escala normalizada (la suma de las áreas del histograma también es aproximadamente 1).
geom_histogram(aes(y = after_stat(density)),breaks = bin_breaks,col = "red",
fill = "blue", alpha = 0.5) +
# Añade la curva de distribución (de color verde)
geom_density(aes(y = after_stat(density)),col = "darkgreen") +
labs(title = "Histograma y Curva de Densidad de Altura",x = "Altura (cm)",y = "Densidad / Frecuencia Normalizada")
print(grafico_con_densidad)

# Reutilizamos el gráfico con densidad y añadimos facet_grid.
grafico_mort <- grafico_con_densidad +
#Facetado: Representa el histograma anterior para cada valor de MORT
# (. ~ MORT) divide el gráfico en columnas por los valores de MORT.
facet_grid(. ~ MORT) + labs(title = "Distribución de la Altura según variable MORT")
print(grafico_mort)
